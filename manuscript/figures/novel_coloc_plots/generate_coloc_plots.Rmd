...
# gtex_distinct_colocs_all <- data.frame()
# for (gwasid in gwas_ids) {
...
#   # get all unique genes from eqtlcat 
#   uniq_eqtlcat_egenes <- eqtl_cat_colocs_by_gwas %>% pull(molecular_trait_id) %>% unique() 
#   
#   # get all unique genes from GTExV8
#   uniq_gtex_egenes <- gtex_colocs_by_gwas %>% pull(molecular_trait_id) %>% unique() 
#   
#   # find which genes are common 
#   common_egenes <- intersect(uniq_eqtlcat_egenes, uniq_gtex_egenes)
#   
#   # find which genes are found only in eqtlcat (without GTExV8)
#   distinct_eqtlcat_egenes[[gwasid]] <- uniq_eqtlcat_egenes[!uniq_eqtlcat_egenes %in% common_egenes]
#   
#   # find which genes are found only in GTExV8
#   distinct_gtex_egenes[[gwasid]] <- uniq_gtex_egenes[!uniq_gtex_egenes %in% common_egenes]
#   
#   # find the colocalisations which are unique to eqtlcat
#   eqtl_cat_distinct_colocs_by_gwas <- eqtl_cat_colocs_by_gwas %>% 
#     filter(molecular_trait_id %in% distinct_eqtlcat_egenes[[gwasid]])
#   
#   # merge distinct eqtlcat colocs per GWAS all together
#   df_novel_eqtlcat_colocs <- rbind(df_novel_eqtlcat_colocs, eqtl_cat_distinct_colocs_by_gwas)
#   
#   # find the colocalisations which are unique to GTExV8
#   gtex_distinct_colocs_by_gwas <- gtex_colocs_by_gwas %>% 
#     filter(molecular_trait_id %in% distinct_gtex_egenes[[gwasid]])
#   
#   # merge distinct eqtlcat colocs per GWAS all together
#   gtex_distinct_colocs_all <- rbind(gtex_distinct_colocs_all, gtex_distinct_colocs_by_gwas)
# }
...
#   left_join(qtl_group_samplesizes %>% select(qtl_subset, sample_size)) %>%
#   left_join(qtlsubset_classes %>% select(qtl_subset, cell_type_class)) %>% 
#   write_tsv("coloc_plots/novel_ld_block_based_counts.tsv")
