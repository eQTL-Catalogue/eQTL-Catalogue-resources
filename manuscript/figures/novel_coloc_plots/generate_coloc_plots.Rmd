...
```{r}
# file_list <- list.files(path = "/Users/kerimov/Work/temp_files/coloc_results_merged/results_coloc_ge_maxbeta/",
#                         pattern = "*GTEx*",recursive = TRUE, full.names = TRUE)
# GTExV8_sign_colocs = data.frame()
# 
...
#   GTExV8_sign_colocs<-rbind(GTExV8_sign_colocs, temp_dataset)
# }
# 
# write_tsv(GTExV8_sign_colocs, "data/GTExV8_sign_colocs_maxbeta.tsv")
```

```{r}
# # select the files which are not GTEx
# file_list <- grep(list.files(path="/Users/kerimov/Work/temp_files/coloc_results_merged/results_coloc_ge_maxbeta/", 
#                              recursive = T, full.names = T), pattern='*GTEx*', invert=TRUE, value=TRUE)
# 
# ge_sign_colocs = data.frame()
# 
# for (file in file_list){
#   print(which(file == file_list))
#   print(file)
...
#   temp_dataset <-read_tsv(file, trim_ws = TRUE)
#   temp_dataset <- temp_dataset %>% dplyr::filter(PP.H4.abf>=0.8)
#   ge_sign_colocs<-rbind(ge_sign_colocs, temp_dataset)
# }

```

```{r}
# file_list <- list.files("/Users/kerimov/Work/temp_files/coloc_results_merged/results_coloc_ma_maxbeta/",
#                         recursive = TRUE, full.names = TRUE)
# ma_sign_colocs = data.frame()
# 
# for (file in file_list){
#   print(which(file == file_list))
# 
#   temp_dataset <-read_tsv(file, trim_ws = TRUE)
#   temp_dataset <- temp_dataset %>% dplyr::filter(PP.H4.abf>=0.8)
#   ma_sign_colocs<-rbind(ma_sign_colocs, temp_dataset)
# }
# 
# ma_pheno_meta <- read_tsv("/Users/kerimov/Work/temp_files/phenotype_metadata/HumanHT-12_V4_Ensembl_96_phenotype_metadata.tsv.gz")
# ma_pheno_meta <- ma_pheno_meta %>%
#   dplyr::rename(molecular_trait_id = phenotype_id) %>%
#   select(molecular_trait_id, gene_id)
# 
# ma_sign_colocs <- ma_sign_colocs %>%
#   left_join(ma_pheno_meta) %>%
#   mutate(molecular_trait_id = gene_id) %>%
#   select(-gene_id)
# 
# eqtlCat_all_sign_colocs <- rbind(ge_sign_colocs, ma_sign_colocs)
# 
# write_tsv(eqtlCat_all_sign_colocs, "data/eqtlCat_all_sign_colocs_maxbeta.tsv")
```
...
qtl_group_samplesizes <- read_tsv("data/qtl_group_samplesizes.tsv")
ld_blocks <-  read_tsv("data/ind_regions_Hg38.bed")
```

```{r}
GTExV8_sign_colocs <- read_tsv("data/GTExV8_sign_colocs_maxbeta.tsv.gz")
eqtlCat_all_sign_colocs <- read_tsv("data/eqtlCat_all_sign_colocs_maxbeta.tsv.gz")
all_sign_colocs <- rbind(GTExV8_sign_colocs, eqtlCat_all_sign_colocs)
all_sign_colocs <- all_sign_colocs %>% mutate(qtl_subset = gsub(pattern = "GTExV8", replacement = "GTEx", x = qtl_subset))
all_sign_colocs$qtl_subset %>% table()
```

```{r}
gwas_ids <- all_sign_colocs$gwas_id %>% unique()
proportions_df <- data.frame()
for (gwasid in gwas_ids) {
  all_colocs_by_gwas <- all_sign_colocs %>% filter(gwas_id == gwasid)
  
  # merge according to chromosome, start and end positions
  all_merged = sqldf::sqldf("
    SELECT *
    FROM all_colocs_by_gwas d1 JOIN ld_blocks d2
    ON d1.chromosome = d2.chr
    AND d1.position >= d2.start
    AND d1.position <= d2.end")
  
  by_ld_block_summarised <- all_merged %>% 
    group_by(ID_hg38) %>% 
    summarise(n_unique_genes_in_ld_block = length(unique(molecular_trait_id)))

  write_tsv(by_ld_block_summarised, paste0("gene_counts_in_ld_blocks/", gwasid,".tsv"))
  
  subtitle_str <- paste0("Prop: proportion of LD blocks containing only one unique gene\nTotal number of LD blocks: ",
                         nrow(by_ld_block_summarised))
  proportion_1 <- round(nrow(by_ld_block_summarised %>% 
                               filter(n_unique_genes_in_ld_block == 1))/nrow(by_ld_block_summarised), digits = 3)*100
  
  proportions_df <- rbind(proportions_df, data.frame(gwas_id = gwasid, prop = proportion_1))
  annotate_label <- paste0(" Prop: ", proportion_1, "%")
  genes_ld_block_plt <- ggplot(by_ld_block_summarised, aes(x=as.factor(n_unique_genes_in_ld_block))) +
    geom_bar(color="black", fill ="#56B4E9") +
    geom_text(stat='count', aes(label=..count..), vjust=-0.5, size = 3) +
    annotate(geom="label", x=-Inf, y=Inf, hjust = -5, vjust=2,label=annotate_label, size = 4, fill = "#ede8a4") +
    labs(title = gwasid, subtitle = subtitle_str) +
    scale_x_discrete(drop=FALSE) +
    ylab("Count of LD blocks") +
    xlab("Number of unique genes assigned to the single LD block") +
    theme_bw()

  ggsave(plot = genes_ld_block_plt, filename = paste0("gene_counts_in_ld_blocks/plots/", gwasid,"_hist.pdf"),
         device = "pdf", height = 5, width = 7)
}

proportions_df %>% write_tsv("gene_counts_in_ld_blocks/proportions_of_one_gene_ld_blocks.tsv")
```


Novel LD-block colocs
```{r}
...
for (gwasid in gwas_ids) {
  # get colocs for the selected GWASid
  eqtlcat_colocs_by_gwas <- eqtlCat_all_sign_colocs %>% filter(gwas_id == gwasid)
  gtex_colocs_by_gwas <- GTExV8_sign_colocs %>% filter(gwas_id == gwasid)
  
  # merge according to chromosome, start and end positions
  # assign LD block ID to all of the GTEx significant colocs
  ...
  # assign LD block ID to all of the eqtlcat significant colocs
  eqtlcat_merged = sqldf::sqldf("
    SELECT *
    FROM eqtlcat_colocs_by_gwas d1 JOIN ld_blocks d2
    ON d1.chromosome = d2.chr
    AND d1.position >= d2.start
    AND d1.position <= d2.end")
  ...
  # find common LD_blocks between eqtlcat and GTEX
  common_ld_blocks <- intersect(gtex_unique_ld_blocks, eqtlcat_unique_ld_blocks)
  
  # find distinct LD_blocks found only in eqtlcat 
  distinct_eqtlcat_ld_blocks[[gwasid]] <- eqtlcat_unique_ld_blocks[!eqtlcat_unique_ld_blocks %in% common_ld_blocks]
  
  # find distinct LD_blocks found only in GTEx 
  distinct_gtex_ld_blocks[[gwasid]] <- gtex_unique_ld_blocks[!gtex_unique_ld_blocks %in% common_ld_blocks]
  
  # get union of all LD_blocks found in both eqtlcat and GTEx
  union_ld_blocks[[gwasid]] <- union(eqtlcat_unique_ld_blocks, gtex_unique_ld_blocks)
  
  # extract colocalisations within LD_blocks found only eqtlCat
  colocs_in_novel_ld_blocks <- eqtlcat_merged %>% 
    filter(ID_hg38 %in% distinct_eqtlcat_ld_blocks[[gwasid]]) %>%
    select(-chr,-start,-end, -ID_hg19)
  
  # merge novel colocs per GWAS into one single dataframe having colocs with all 14 GWASs
  novel_ld_block_colocs <- novel_ld_block_colocs %>% rbind(colocs_in_novel_ld_blocks)
}

write_tsv(novel_ld_block_colocs, "data/novel_eqtlcat_ge_colocs_in_novel_ldblocks_relative_to_GTExV8.tsv")
```

Lymphocyte count scatter plot
```{r}
# Make scatter plots per GWAS (sample size in X, novel ld block counts in Y)
qtlsubset_classes <- read_tsv("data/qtl_subset_classes.tsv")

# summarise number of unique LD blocks and coloc counts per GWAS and dataset 
novel_ld_block_based_counts <- novel_ld_block_colocs %>% 
  group_by(gwas_id, qtl_subset) %>% 
  summarise(n_colocs_in_novel_ld_blocks = n(), 
            n_unique_novel_ld_blocks = length(unique(ID_hg38))) 

# set gwasid to Lymphocyte Count
gwasid = "LC-ebi-a-GCST004627"

# extract summarised counts for Lymphocyte Count
gwas_colocs = novel_ld_block_based_counts %>% filter(gwas_id == gwasid)

# add the datasets which does not have any colocs in novel ld blocks
...
# merge all the dataset coloc counts together add sample_size and cell_type_class info
...
ggplotly_plot <- plotly::ggplotly(temp_plot)
  htmlwidgets::saveWidget(widget = plotly::as_widget(ggplotly_plot),
                          file = file.path(normalizePath(paste0(output_dir, "/plotly")),
                                           paste0("LC_scatter_plot.html")),
                          libdir = "dependencies")


ggsave(filename = "manuscript_needed_plots/LC_novel_coloc_plot_ge.pdf", plot = temp_plot, width = 3, height = 3)
```
...
temp_plot <- ggplot(gwas_colocs, aes(x = sample_size, y = n_unique_novel_ld_blocks, label=qtl_subset)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
temp_plot
ggsave("manuscript_needed_plots/Height_novel_coloc_plot_ge.pdf", plot = temp_plot, width = 3, height = 3)
```
...
novel_ld_block_based_counts %>%
  left_join(qtl_group_samplesizes %>% select(qtl_subset, sample_size)) %>%
  left_join(qtlsubset_classes %>% select(qtl_subset, cell_type_class)) %>%
  write_tsv("data/novel_ld_block_based_counts_ge.tsv")
...
for (gwasid in gwas_ids) {
  ...
  # arrange by descending n_unique_novel_ld_blocks so we can label the datasets with highest number of novel ld blocks
...
# summarise number of unique LD blocks and coloc counts per GWAS
novel_ld_block_based_counts_prop <- novel_ld_block_colocs %>% 
  group_by(gwas_id) %>% 
  summarise(n_colocs_in_novel_ld_blocks = n(), 
            n_unique_novel_ld_blocks = length(unique(ID_hg38)))

# get number of unique LD_blocks found in any of eqtlcat or GTEx (per GWAS)
...
# get number of unique LD_blocks found in GTEx only (per GWAS)
...
write_tsv(novel_ld_block_counts, "data/novel_ld_blocks_counts_by_gwas_ge.tsv")
...
ggsave("manuscript_needed_plots/novel_LD_block_proportions.pdf", plot = plot_distinct_counts, width = 7, height = 4, device = "pdf")
...
#read gwas and dataset meta classes
gwas_classes <- read_tsv("data/gwas_classes.tsv")
qtlsubset_classes <- read_tsv("data/qtl_subset_classes.tsv")

# get summarised novel LD block counts and colocs in them 
# normalise by sample size of the dataset
...
for (gwasid in gwas_ids) {
  ...
  # generate zero counts dataframe for datasets which does not have any novel colocs for the selected GWAS
...
```{r}
# Filter out the datasets with smaller sample size than 150
...
temp_plot_violin <-
  ggplot(
    novel_ld_block_based_counts_all_gwas_filt,
    aes(
      x = sample_size_norm_novel_LD_block_count,
      y = gwas_id_short,
      label = qtl_subset,
      fill = present_in_GTEx,
      color = present_in_GTEx
    )
  ) +
  geom_violin(position = "dodge", alpha = 0.8) +
  theme_bw() +
  scale_color_manual(
    values = c("cadetblue3", "darkseagreen2"),
    name = "Cell type\npresent in GTEx",
    labels = c("No", "Yes")
  ) +
  scale_fill_manual(
  ...
  geom_jitter(
    mapping = aes(color = NULL, group = present_in_GTEx),
    position = position_dodge(0.8),
    size = 1,
    shape = "triangle",
    alpha = 0.9
  ) +
  facet_grid(gwas_class ~ .,
             shrink = TRUE,
             scales = "free_y",
             space = "free_y") +
  ggplot2::labs(x = "Novel LD block count / sample size",
...
ggsave("manuscript_needed_plots/coloc_violin.pdf", plot = temp_plot_violin, width = 5, height = 6.5)

ggplotly_plot <- plotly::ggplotly(temp_plot_violin)
htmlwidgets::saveWidget(
  widget = plotly::as_widget(ggplotly_plot),
  file = file.path(normalizePath(paste0(
    output_dir, "/plotly"
  )), "violin_plots.html"),
  libdir = "dependencies"
)
...
save_ggplots(
  plot = temp_plot_violin,
  path = paste0(output_dir, "/faceted_difference_violins/"),
  filename = "GTExV8_vs_eQTLCatR3_violin_ss_filter_label_no_label",
  width = 20
)
...
save_ggplots(
  plot = temp_plot_violin_labeled,
  path = paste0(output_dir, "/faceted_difference_violins/"),
  filename = "GTExV8_vs_eQTLCatR3_violin_ss_filter_label",
  width = 20
)
```
