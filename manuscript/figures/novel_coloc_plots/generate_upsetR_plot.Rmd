...
# Metadata file can be found in https://zenodo.org/record/3366011#.YHx4WJMzbeI
tx_pheno_meta <- read_tsv("/Users/kerimov/Work/R_env/data/phenotype_metadata/transcript_usage_Ensembl_96_phenotype_metadata.tsv.gz") %>%
  select(phenotype_id, gene_id)
eqtlCat_rnaseq_sign_colocs_tx <- left_join(eqtlCat_rnaseq_sign_colocs_tx, tx_pheno_meta, 
                                           by = c("molecular_trait_id" = "phenotype_id"))

txrev_pheno_meta <- read_tsv("/Users/kerimov/Work/R_env/data/phenotype_metadata/txrevise_Ensembl_96_phenotype_metadata.tsv.gz") %>%
  select(phenotype_id, gene_id)
eqtlCat_rnaseq_sign_colocs_txrev <- left_join(eqtlCat_rnaseq_sign_colocs_txrev, txrev_pheno_meta, 
                                              by = c("molecular_trait_id" = "phenotype_id"))

exon_pheno_meta <- read_tsv("/Users/kerimov/Work/R_env/data/phenotype_metadata/exon_counts_Ensembl_96_phenotype_metadata.tsv.gz", 
                            col_types = "cccccddccccddn") %>% select(phenotype_id, gene_id)
eqtlCat_rnaseq_sign_colocs_exon <- left_join(eqtlCat_rnaseq_sign_colocs_exon, exon_pheno_meta, 
                                             by = c("molecular_trait_id" = "phenotype_id"))

eqtlCat_rnaseq_sign_colocs_ge <- eqtlCat_rnaseq_sign_colocs_ge %>% mutate(gene_id = molecular_trait_id)
