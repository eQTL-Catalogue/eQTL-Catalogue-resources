...
extract_novel_ld_blocks <- function(GTExV8_sign_colocs, eqtlCat_all_sign_colocs)  {
  gwas_ids <- GTExV8_sign_colocs %>% 
    filter(gwas_id != "RA-ieu-a-833") %>% 
    pull(gwas_id) %>% unique()
...
```{r}
novel_ld_blocks_ge <- extract_novel_ld_blocks(GTExV8_sign_colocs = GTExV8_sign_colocs_ge, eqtlCat_rnaseq_sign_colocs_ge)
novel_ld_blocks_tx <- extract_novel_ld_blocks(GTExV8_sign_colocs = GTExV8_sign_colocs_ge, eqtlCat_rnaseq_sign_colocs_tx_geneid)
novel_ld_blocks_txrev <- extract_novel_ld_blocks(GTExV8_sign_colocs = GTExV8_sign_colocs_ge, eqtlCat_rnaseq_sign_colocs_txrev_geneid)
novel_ld_blocks_exon <- extract_novel_ld_blocks(GTExV8_sign_colocs = GTExV8_sign_colocs_ge, eqtlCat_rnaseq_sign_colocs_exon_geneid)
```

```{r}
listInput <- list(ge = novel_ld_blocks_ge$ID_hg38, 
                  tx = novel_ld_blocks_tx$ID_hg38, 
                  txrev = novel_ld_blocks_txrev$ID_hg38,
                  exon = novel_ld_blocks_exon$ID_hg38)
...
eqtlcat_ldblocks_ge = sqldf::sqldf("
      SELECT *
...
eqtlcat_ldblocks_tx = sqldf::sqldf("
      SELECT *
...
eqtlcat_ldblocks_txrev = sqldf::sqldf("
      SELECT *
...
eqtlcat_ldblocks_exon = sqldf::sqldf("
      SELECT *
...
unique(eqtlcat_ldblocks_ge$ID_hg38) %>% length() 
unique(eqtlcat_ldblocks_tx$ID_hg38) %>% length() 
unique(eqtlcat_ldblocks_txrev$ID_hg38) %>% length() 
unique(eqtlcat_ldblocks_exon$ID_hg38) %>% length() 
```

```{r}
listInputUniqueLDBlockCount <- list(ge = eqtlcat_ldblocks_ge$ID_hg38, 
                  tx = eqtlcat_ldblocks_tx$ID_hg38, 
                  txrev = eqtlcat_ldblocks_txrev$ID_hg38,
                  exon = eqtlcat_ldblocks_exon$ID_hg38)
...
df_novel_ld_blocks_per_gwas_ge <- data.frame()
df_novel_ld_blocks_per_gwas_tx <- data.frame()
df_novel_ld_blocks_per_gwas_txrev <- data.frame()
df_novel_ld_blocks_per_gwas_exon <- data.frame()

upset_plot_list_gwas <- list()
for (gwasid in gwas_ids) {
  eqtlcat_ldblocks_ge_gwas <- eqtlcat_ldblocks_ge %>% filter(gwas_id == gwasid) %>% mutate(ldgwas = paste0(gwasid,"_", ID_hg38))
  eqtlcat_ldblocks_tx_gwas <- eqtlcat_ldblocks_tx %>% filter(gwas_id == gwasid) %>% mutate(ldgwas = paste0(gwasid,"_", ID_hg38))
  eqtlcat_ldblocks_txrev_gwas <- eqtlcat_ldblocks_txrev %>% filter(gwas_id == gwasid) %>% mutate(ldgwas = paste0(gwasid,"_", ID_hg38))
  eqtlcat_ldblocks_exon_gwas <- eqtlcat_ldblocks_exon %>% filter(gwas_id == gwasid) %>% mutate(ldgwas = paste0(gwasid,"_", ID_hg38))
  
  df_novel_ld_blocks_per_gwas_ge <- df_novel_ld_blocks_per_gwas_ge %>% rbind(eqtlcat_ldblocks_ge_gwas)
  df_novel_ld_blocks_per_gwas_tx <- df_novel_ld_blocks_per_gwas_tx %>% rbind(eqtlcat_ldblocks_tx_gwas)
  df_novel_ld_blocks_per_gwas_txrev <- df_novel_ld_blocks_per_gwas_txrev %>% rbind(eqtlcat_ldblocks_txrev_gwas)
  df_novel_ld_blocks_per_gwas_exon <- df_novel_ld_blocks_per_gwas_exon %>% rbind(eqtlcat_ldblocks_exon_gwas)
  
  
  listInputUniqueLDBlockCountGWAS <- list(ge = eqtlcat_ldblocks_ge_gwas$ldgwas,
                                              tx = eqtlcat_ldblocks_tx_gwas$ldgwas,
                                              txrev = eqtlcat_ldblocks_txrev_gwas$ldgwas,
                                              exon = eqtlcat_ldblocks_exon_gwas$ldgwas)
...
listInputUniqueLDBlockCount_per_gwas <- list(ge = df_novel_ld_blocks_per_gwas_ge$ldgwas,
                                              tx = df_novel_ld_blocks_per_gwas_tx$ldgwas,
                                              txrev = df_novel_ld_blocks_per_gwas_txrev$ldgwas,
                                              exon = df_novel_ld_blocks_per_gwas_exon$ldgwas)

write_rds(listInputUniqueLDBlockCount_per_gwas, "../data/listInputUniqueLDBlockCount_per_gwas.rds")
