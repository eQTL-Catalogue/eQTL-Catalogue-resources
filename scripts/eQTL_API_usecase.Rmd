...
plot_faceted_multi_manhattans <- function(merged_eqtl_assocs,
  ...
  plot_gwas_eqtl_plot = plot_base_all +
    geom_point(aes(colour = factor(is_lead_var))) +
    ...
    ) + scale_color_manual(values=c("skyblue4", "firebrick4", "chartreuse4"))
...
save_ggplots(plot = joint_ge_plot, path = figures_dir, filename = "ge_merged_plot_with_exon", height = 24, width = 15)
joint_ge_plot
```
...
joint_tx_plot = cowplot::plot_grid(manhattan_tx_gwas, CD40_exons_plot,
                                align = "v", ncol = 1, rel_heights = c(5,2))

save_ggplots(plot = joint_tx_plot, path = figures_dir, filename = "tx_merged_plot_with_exon", height = 11.5, width = 15)
joint_tx_plot
```
